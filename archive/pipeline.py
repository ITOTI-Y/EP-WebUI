...
def createIDF(city, ssp, btype, ECM_dict):
    idf_name = IDFname(ECM_dict)
    ...
    if ECM_dict['lighting'] != 0:
        ...
        # print(reduct_rate)
        ECMs.writeLightings(ECM_dict['lighting'], new_IDF)
    os.remove(folder+"base.idf")
    return new_IDF

def getResult(city, ssp, btype, ECM_dict={}, optimization_rate=0):
    if not ECM_dict:
        folder = epsim_folder + city + '\\%s\\' % (ssp)
...
def get_sourceEUI(city, ssp, btype, ECM_dict={}):
    if not ECM_dict:
        folder = epsim_folder + city + '\\%s\\' % (ssp)
        f = [folder + f for f in os.listdir(folder) if f.endswith('Table.csv') if btype in f][0]
    else:
...
def cal_baseline_city_EUI(city):
    ...
    for ssp in ['TMY', 126, 245, 370, 585]:
        mean_EUI = 0            
        for btype in ['OfficeLarge', 'OfficeMedium', 'ApartmentHighRise', 'SF', 'MF']:   
...
def runEP(city, ssp, btype, ECM_dict = {}):
    if not ECM_dict:
...
def optimizeAllBuildings(city):
    ...
    for ssp in ['TMY', 126, 245, 370, 585]:
        ...
        for btype in ['OfficeLarge', 'ApartmentHighRise', 'OfficeMedium', 'SF', 'MF']:
...
def analysis(city):
    ...
    for ssp in ['TMY', 126, 245, 370, 585]:
        ...
        for btype in ['OfficeLarge', 'OfficeMedium', 'ApartmentHighRise', 'SF', 'MF']:
            for f in optimized_files:
                if "optimized_%s_%s_%s" % (btype, city, ssp) in f:
    ...
    btypes = ['OfficeLarge', 'OfficeMedium', 'ApartmentHighRise', 'SF', 'MF']
    features = ['Intercept', 'shgc', 'win_U', 'nv_area', 'insu', 'infl', 'cool_COP', 'cool_air_temp', 'lighting']
    multi_index = pd.MultiIndex.from_product([btypes, features, ssps], names=['Building Type', 'Features', 'SSP'])
    ...
    coef_df = coef_df.reindex(index=['OfficeLarge', 'OfficeMedium', 'ApartmentHighRise', 'SF', 'MF'], level=0)
...
def Para_dict():
    ...
    DOE_dict['nv_area'] = [0.4, 0.8, 1.2, 1.6, 2, 2.4] #m2
...
def main():
    ...
    global climatezone_dict, NG_source_energy_conversion, btype_ratio_dict
    ...
    #For running
    city = 'Chicago'
    Parallel(n_jobs = cpu_count - 4, verbose = 10)(delayed(runEP)(city, ssp, btype) for ssp in ['TMY', 126, 245, 370, 585] for btype in ['OfficeLarge', 'OfficeMedium', 'ApartmentHighRise', 'SF', 'MF'])
